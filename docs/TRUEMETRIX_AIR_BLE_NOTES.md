...
also present — not part of the standard profile, not investigated
...
UUIDs) — **no write was ever sent to RACP (`0x2A52`)**. The meter
...
indication was observed on RACP in this capture — likely just needs an
idle/silence timeout after the last notification, or reading `0x2A51`
Glucose Feature / a known record count some other way. Worth confirming
with a longer capture or an explicit RACP "number of stored records"
read on a future session).
...
meter doesn't populate that data, only exists because the standard flag
